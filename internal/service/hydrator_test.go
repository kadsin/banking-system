...
func TestHydratorServiceRepopulateFromSnapshotAndQueueWithoutCommit(t *testing.T) {
	q := queue.New()
	balanceRepo := datalayer.NewBalanceRepository(cache.New())
	hydratorRepo := datalayer.NewHydratorRepository(cache.New())
	svc := NewHydratorService(balanceRepo, hydratorRepo, q, "tx")
	...
	_, err = publishTx(q, "tx", domain.Transaction{FromAccountID: accountID, ToAccountID: otherID, Amount: 50}) // offset 0
	require.NoError(t, err)
	_, err = publishTx(q, "tx", domain.Transaction{FromAccountID: otherID, ToAccountID: accountID, Amount: 200}) // offset 1
	require.NoError(t, err)
	_, err = publishTx(q, "tx", domain.Transaction{FromAccountID: accountID, ToAccountID: otherID, Amount: 30}) // offset 2
	require.NoError(t, err)
	...
	stored, err := balanceRepo.Get(accountID)
	require.NoError(t, err)
